...
from yolov5_master.utils.general import probability_output
...
def attack(model_ori, x, data_lb, data_ub, vnnlib, verified_status, verified_success, crown_filtered_constraints=None, initialization='uniform'):
    ...
    if attack_ret:
        ...
        xx = attack_images[0]
        for k in range(min(xx.shape[0],5)):
            img = transform(xx[k])
            #img_array = np.uint8(img_array / np.max(img_array) * 255)
            for i in range(1, 999):
                results_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'images_results/after' + str(i) + '.png')
...
def test_conditions(input, output, C_mat, rhs_mat, cond_mat, same_number_const, data_max, data_min):
    ...
    if same_number_const:
        ...
        #cond = torch.matmul(C_mat[:,:,:,:,:output.shape[1]], output.unsqueeze(-1).permute(0,3,2,1,4)).squeeze(-1).squeeze(1) - rhs_mat
...
def gen_adv_example(model_ori, x, best_deltas, data_max, data_min, C_mat, rhs_mat, cond_mat):
    ...
    attack_output = model_ori(attack_image.view(-1, *x.shape[1:])).unsqueeze(0)
    ...
    for i in range(attack_margin.shape[-1]):
        if attack_margin[-1,-1,i] < 0:
            transform = T.ToPILImage()
            img = transform(attack_image[-1,i,:,:,:])
            for j in range(1, 999):
                results_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                            'images_results/attack_image' + str(j) + '.png')
                if not os.path.exists(results_path):
                    break
            img.save(results_path)
            break
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat, cond_mat, same_number_const, alpha, 
    ...
    for _ in range(attack_iters):
        ...
        output = model(inputs.view(-1, *input_shape[1:]))
        output = output.view(input_shape[0], *extra_dim, num_classes)
        ...
        #loss_gama.sum().backward(allow_unreachable=True, accumulate_grad=True)
...
def attack_with_general_specs(model, x, data_min, data_max, list_target_label_arrays, initialization="uniform", GAMA_loss=False):
    ...
    #output = model(x, augment=False, visualize=False).detach()
