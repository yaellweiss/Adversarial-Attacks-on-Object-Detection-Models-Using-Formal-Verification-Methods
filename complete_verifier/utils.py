...
from yolov5_master.utils.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
...
def load_model(weights_loaded=True):
    ...
    if arguments.Config['model']['name'] is not None:
        ...
        if arguments.Config["model"]["name"] == "yolo":

            from yolov5_master.models.common import DetectMultiBackend
            from yolov5_master.utils.general import check_img_size

            sd = torch.load(arguments.Config["model"]["path"], map_location=torch.device('cpu'))

            model = DetectMultiBackend(weights='yolov5s.pt', device='cpu', dnn=False, data=None, fp16=False)
            bs = 1
            ...
            model.model.load_state_dict(torch.load(weights_path)['model'].state_dict())

        elif arguments.Config["model"]["name"] == "ssd":
            sd = torch.load(arguments.Config["model"]["path"], map_location=torch.device('cpu'))
            #sd = torch.load(arguments.Config["model"]["path"], map_location=torch.device('cpu'))
            #model = torchvision.models.detection.ssd300_vgg16(weights=torchvision.models.detection.SSD300_VGG16_WEIGHTS)
            if 'state_dict' in sd:
                sd = sd['state_dict']
...
def load_sdp_dataset(eps_temp=None):
    ...
    if arguments.Config["data"]["dataset"] == "CIFAR_SDP":
        ...
        da = np.load(os.path.join(database_path, "mnist/y_sdp.npy"))
    ...
    elif arguments.Config["data"]["dataset"] == "YOLO_IMG":
        X = np.load(os.path.join(database_path, "mnist/X_sdp.npy"))
...
def load_ssd_dataset(eps_temp=None):
    import torchvision.transforms as T
    from PIL import Image
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets/ssd')
    X = np.load(os.path.join(database_path, "X_ssd.npy"), allow_pickle=True)
    X = torch.from_numpy(X.astype(np.float32))
    X = X / 255.0
    """
    ...
    for i in range (X.shape[0]):
        ...
        for j in range(1, 999):
            results_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'before_attack/before_attack' + str(j) + '.png')
            if not os.path.exists(results_path):
                break
        img.save(results_path)
    """
    l = np.load(os.path.join(database_path, "y_ssd.npy"), allow_pickle=True)
    labels = torch.zeros((len(l), 1, 6))
    for i, tensor in enumerate(l):
        labels[i] = tensor
    runnerup = np.copy(labels)
    if eps_temp is None: eps_temp = 0.3
...
def load_yolo_dataset(eps_temp=None):
    import torchvision.transforms as T
    from PIL import Image
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets/yolo')
    X = np.load(os.path.join(database_path,"X_yolo.npy"), allow_pickle=True)
    X = torch.from_numpy(X.astype(np.float32))
    X = X/255.0
    ...
    l = np.load(os.path.join(database_path, "y_yolo.npy"), allow_pickle=True)
    labels = torch.zeros((len(l), 1,6))
    for i,tensor in enumerate(l):
        labels[i] = tensor
    runnerup = np.copy(labels)
...
def load_verification_dataset(eps_before_normalization):
    ...
    elif "YOLO_IMG" in arguments.Config["data"]["dataset"]:
        # YOLO image dataset.
        data_config = load_yolo_dataset(eps_temp=eps_before_normalization)
    elif "SSD_IMG" in arguments.Config["data"]["dataset"]:
        data_config = load_ssd_dataset(eps_temp=eps_before_normalization)
    else:
...
def construct_vnnlib(X, labels, runnerups, data_max, data_min, perturb_epsilon, target_labels, example_idx_list):
    ...
    for i in range(len(example_idx_list)):
        label = labels[example_idx_list[i]][:,5]
        #label=label.reshape(1,-1)
        this_x_range = x_range[i]
        print(labels[example_idx_list[i]])
        if(label.shape[0] == 1):
            label=torch.tensor([int(label.item())])
            ...
            if arguments.Config["data"]["num_outputs"] > 1:
                # Multi-class
                if arguments.Config["specification"]["robustness_type"] == "verified-acc":
                    c = np.zeros([1, num_outputs, num_outputs, 100])
